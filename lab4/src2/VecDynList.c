...
VecDyn * insertVecDyn(VecDyn * vecDyn, int val)
{
  VecDyn *new;
  if(vecDyn == NULL){
    vecDyn = (VecDyn*)malloc(sizeof(VecDyn));
    vecDyn->size = 1;
    vecDyn->free = 1;
    vecDyn->table = (int*)malloc(sizeof(int));
    vecDyn->next = NULL;
  }
  if(vecDyn->free == 0){
    new = (VecDyn*)malloc(sizeof(VecDyn));//!falta verificação
    new->size = 2*(vecDyn->size);
    new->free = (new->size) - 1;
    new->table = (int*)malloc(sizeof(int)*new->size);
    new->next = vecDyn;
    (new->table)[0] = val;
  }else{
    (vecDyn->table)[(vecDyn->size) - (vecDyn->free)] = val;
    vecDyn->free = vecDyn->free -1;
    new = vecDyn;
  }
  return new;
}
 ...
 *
 */
void modifyVecDynValue(VecDyn * vecDyn, int idx, int val)//!logN + 1
{
  VecDyn *vec_loop = vecDyn;
  if (idx < vec_loop->size-1 ){
    modifyVecDynValue(vec_loop->next, idx, val);
  }else{
    vec_loop->table[idx - vec_loop->size + 1] = val;
  }
   return;
}
